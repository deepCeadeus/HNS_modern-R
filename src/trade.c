...
static bool8 DoTradeAnim_Cable(void)
{
    ...
    case STATE_SHOW_NEW_MON:
        ...
        CreatePokeballSpriteToReleaseMon(sTradeAnim->monSpriteIds[TRADE_PARTNER], gSprites[sTradeAnim->monSpriteIds[TRADE_PARTNER]].oam.paletteNum, 120, 84, 2, 1, 20, PALETTES_BG | (0xF << 16), sTradeAnim->monSpecies[TRADE_PARTNER], sTradeAnim->monBallIds[TRADE_PARTNER]);        FreeSpriteOamMatrix(&gSprites[sTradeAnim->bouncingPokeballSpriteId]);
...
static bool8 DoTradeAnim_Wireless(void)
{
    ...
    case STATE_BYE_BYE:
        if (++sTradeAnim->timer == 80)
        {
            sTradeAnim->releasePokeballSpriteId = CreateTradePokeballSprite(sTradeAnim->monSpriteIds[TRADE_PLAYER], gSprites[sTradeAnim->monSpriteIds[TRADE_PLAYER]].oam.paletteNum, 120, 32, 2, 1, 0x14, 0xfffff, sTradeAnim->monBallIds[TRADE_PLAYER]);            sTradeAnim->state++;
            StringExpandPlaceholders(gStringVar4, gText_ByeByeVar1);
            DrawTextOnTradeWindow(0, gStringVar4, 0);
        }
        break;
